...
from aidd_worker.services.vina_service import (
    execute_docking_job,
    EGFR_4WKQ_RECEPTOR_PDBQT, SYNTHETIC_LIGAND_FIXTURE_B_PDBQT
)
...
def test_native_vina_docking_subprocess():
    ...
    req = DockingJobRequest(
        ...
        ligands=[{"id": "LIG-FIXTURE-B", "name": "Synthetic ligand fixture B", "pdbqt": SYNTHETIC_LIGAND_FIXTURE_B_PDBQT}],
    ...
    assert meta["vina_binary_digest_verified"] is True
    assert meta["vina_binary_sha256"] == meta["vina_expected_binary_sha256"]
    assert meta["vina_path_verified"] is True
    assert meta["vina_package_metadata_verified"] is True
    assert results[0]["output_pdbqt_hash"]
    assert results[0]["ligand_output_integrity"]["verified"] is True
